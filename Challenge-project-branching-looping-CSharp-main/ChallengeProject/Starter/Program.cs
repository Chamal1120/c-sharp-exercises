...
for (int i = 0; i < maxPets; i++) {
  ...
  case 0:
    ...
    animalPhysicalDescription =
        "medium sized cream colored female golden retriever weighing about " +
        "65 pounds. housebroken.";
  ...
  case 1:
    ...
    animalPersonalityDescription =
        "loves to have his ears rubbed when he greets you at the door, or at " +
...
do {
  ...
  if (readResult != null) {
    ...
    // ensure a valid entry, but we
    //  use a conditional statement below that only processes the valid entry
    //  values, so the do statement is not required here.
  }
  ...
  case "2":
    ...
    while (anotherPet == "y" && petCount < maxPets) {
      ...
      // build the animal the ID number - for example C1, C2, D3 (for Cat 1, Cat
      // 2, Dog 3)
      ...
      do {
        Console.WriteLine("Enter a physical description of the pet (size, " +
                          "color, gender, weight, housebroken)");
        readResult = Console.ReadLine();
        if (readResult != null) {
      ...
      ourAnimals[petCount, 4] =
          "Physical description: " + animalPhysicalDescription;
      ourAnimals[petCount, 5] = "Personality: " + animalPersonalityDescription;

      // increment petCount (the array is zero-based, so we increment the
      // counter after adding to the array)
      petCount = petCount + 1;
      ...
      // NOTE: The value of anotherPet (either "y" or "n") is evaluated in the
      // while statement expression - at the top of the while loop
    }
    ...
    for (int i = 0; i < maxPets; i++) {
      if (ourAnimals[i, 0] != "ID #: ") {
        if (ourAnimals[i, 2] == "Age: " || ourAnimals[i, 2] == "Age: ?") {
          Console.WriteLine(
              $"Age is empty for animal ID: {ourAnimals[i, 0].Substring(6)}!\n");
          validEntry = false;
          do {
        ...
        if (ourAnimals[i, 4] == "Physical description: ") {
          Console.WriteLine(
              $"Physical descriptions are empty for animal ID: {ourAnimals[i, 0].Substring(5)}!\n");
          validEntry = false;
          do {
            Console.Write("Enter a physical description: ");
            readResult = Console.ReadLine();
            if (readResult != null) {
              validEntry = true;
    ...
    for (int i = 0; i < maxPets; i++) {
      if (ourAnimals[i, 0] != "ID #: ") {
        if (ourAnimals[i, 3] == "Nickname: ") {
          Console.WriteLine(
              $"Nickname is empty for animal ID: {ourAnimals[i, 0].Substring(6)}!\n");
          validEntry = false;
        ...
        if (ourAnimals[i, 5] == "Personality: ") {
          Console.WriteLine(
              $"Personality is empty for animal ID: {ourAnimals[i, 0].Substring(6)}!\n");
          do {
